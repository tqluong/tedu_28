...
public class LoginController {
	...
	public String processLogin(Model model , @ModelAttribute("loginForm")@Validated User users , BindingResult result , HttpSession session) {
		...
		for(Object obj : role.getAuths()) {
			...
			if(menu.getParrentId()==0 && menu.getOrderIndex()!=-1 && menu.getActiveFlag()==1 && auth.getPermission()==1 && auth.getActiveFlag()==1) {
				menu.setIdMenu(menu.getUrl().replace("/", "")+"Id"); 
				menuList.add(menu);
			}else if( menu.getParrentId()!=0 && menu.getOrderIndex()!=-1 && menu.getActiveFlag()==1 && auth.getPermission()==1 && auth.getActiveFlag()==1) {
				menu.setIdMenu(menu.getUrl().replace("/", "")+"Id"); 
		...
		for(Menu menu : menuList) {
			...
			for(Menu childMenu : menuChildList) {
				if(childMenu.getParrentId()== menu.getId()) {
